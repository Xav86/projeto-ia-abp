import json
import streamlit as st
from charts.gauge import render_gauge
from charts.fuzzy import render_fuzzy
from charts.fuzzy_inputs import render_fuzzy_inputs
from components.fuzzy_engine import calcular_risco

st.set_page_config(
    page_title="Modo Analítico — Incêndios Florestais",
    page_icon="🔥",
    layout="wide",
)

with open("config/defaults.json", encoding="utf-8") as f:
    DEFAULTS = json.load(f)

if "risco" not in st.session_state:
    st.session_state.risco         = DEFAULTS["risco"]
if "probabilidade" not in st.session_state:
    st.session_state.probabilidade = DEFAULTS["probabilidade"]
if "justificativa" not in st.session_state:
    st.session_state.justificativa = DEFAULTS["justificativa"]
if "temperatura" not in st.session_state:
    st.session_state.temperatura   = 25
if "umidade" not in st.session_state:
    st.session_state.umidade       = 50
if "vento" not in st.session_state:
    st.session_state.vento         = 20

# ── Cabeçalho ────────────────────────────────────────────────────────────────
st.markdown("## 🔥 Sistema Inteligente de Previsão de Incêndios Florestais")
st.caption("Modo analítico — funções de pertinência de todas as variáveis")
st.divider()

# ── Linha 1: Entradas | Gauge ─────────────────────────────────────────────────
col_esq, col_dir = st.columns([1, 2], gap="medium")

with col_esq:
    st.markdown("### Parâmetros Climáticos")

    temperatura = st.slider("Temperatura (°C)",        0,  50, st.session_state.temperatura, 1)
    umidade     = st.slider("Umidade (%)",              0, 100, st.session_state.umidade,     1)
    vento       = st.slider("Velocidade do vento (km/h)", 0, 100, st.session_state.vento,    1)

    st.divider()

    col_m1, col_m2 = st.columns(2)
    col_m1.metric("Risco",        st.session_state.risco)
    col_m2.metric("Probabilidade", f"{st.session_state.probabilidade:.0f}%")

    if st.button("🔍 Calcular Risco", use_container_width=True, type="primary"):
        resultado = calcular_risco(temperatura, umidade, vento)
        st.session_state.risco         = resultado["risco"]
        st.session_state.probabilidade = resultado["probabilidade"]
        st.session_state.justificativa = resultado["justificativa"]
        st.session_state.temperatura   = temperatura
        st.session_state.umidade       = umidade
        st.session_state.vento         = vento
        st.rerun()

with col_dir:
    st.markdown("### Indicador de Risco")
    st.plotly_chart(render_gauge(st.session_state.probabilidade), use_container_width=True)

st.divider()

# ── Linha 2: Justificativa | Gráfico de saída ────────────────────────────────
col_esq2, col_dir2 = st.columns([1, 2], gap="medium")

with col_esq2:
    st.markdown("### Justificativa")
    st.info(st.session_state.justificativa, icon="📋")

with col_dir2:
    st.markdown("### Função de Pertinência — Risco (saída)")
    st.pyplot(render_fuzzy(st.session_state.probabilidade), use_container_width=True)

st.divider()

# ── Linha 3: Pertinências das entradas ───────────────────────────────────────
st.markdown("### Funções de Pertinência — Variáveis de Entrada")
st.caption(
    f"Linha tracejada: valor atual usado no último cálculo  "
    f"(temp={st.session_state.temperatura}°C  "
    f"umid={st.session_state.umidade}%  "
    f"vento={st.session_state.vento} km/h)"
)
st.pyplot(
    render_fuzzy_inputs(
        st.session_state.temperatura,
        st.session_state.umidade,
        st.session_state.vento,
        st.session_state.probabilidade,
    ),
    use_container_width=True,
)
